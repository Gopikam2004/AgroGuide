...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT * FROM buyerregistration WHERE buyer_mail = '$email'";
    ...
    if ($count_rows > 0) {
        ...
        $update_query = "UPDATE buyerregistration SET reset_token = '$token', reset_token_expiry = '$expiry' WHERE buyer_mail = '$email'";
        ...
        try {
            ...
            //Recipients
            $mail->setFrom('agro123guide@gmail.com', 'AgroGuideBuyer Support');
            $mail->addAddress($email); // Email address where you want to send

            //Content
            $mail->isHTML(true);
            $mail->Subject = 'Password Reset Request';
            $resetLink = "https://localhost/AgroGuide-MiniProject/auth/reset_passwordbuyer.php?token=" . urlencode($token);
